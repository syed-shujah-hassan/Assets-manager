...
export default function PrivacySecurityScreen() {
  ...
  useEffect(() => {
    loadPrivacySettings('citizen').then(setSettings);
  }, []);
  ...
  const handleSavePrivacy = async () => {
    ...
    try {
      await savePrivacySettings('citizen', settings);
...
const styles = StyleSheet.create({
  ...
  outlineText: { color: Colors.teal, fontSize: 15, fontFamily: 'Inter_600SemiBold' },
});
